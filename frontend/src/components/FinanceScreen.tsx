...
export const FinanceScreen: React.FC<FinanceScreenProps> = ({
  ...
  return (
    <div className="flex flex-col w-full px-4 space-y-4 max-w-max-content-width mx-auto pb-4 pt-1">
      ...
      {/* SECTION 4: SMART CAMPUS SHOPPING (Strictly Filtered for ₹2,500 Balance) */}
      <section className="space-y-3">
        ...
        <div className="space-y-3">
          {shoppingItems.map((item) => (
            <div
              key={item.id}
              className={`p-3.5 rounded-2xl bg-surface-container-lowest shadow-sm flex items-center gap-3.5 transition-all border ${
                item.selected
                  ? 'border-primary/40 bg-surface-container-low/40'
                  : 'border-outline-variant/15'
              }`}
            >
              <div className="w-18 h-18 rounded-xl overflow-hidden bg-surface-container shrink-0 shadow-xs">
                <img
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-1">
                  <h3 className="text-[14px] font-bold text-on-surface truncate">{item.name}</h3>
                  <span className="text-[14px] font-bold text-primary shrink-0">₹{item.price}</span>
                </div>
                <p className="text-[11px] text-on-surface-variant truncate mt-0.5">{item.description}</p>
        ...
        <div className="p-4 rounded-2xl bg-surface-container-low shadow-sm space-y-3 border border-outline-variant/20">
          <div className="flex items-center justify-between">
