...
export const ExploreScreen: React.FC<ExploreScreenProps> = ({ spots }) => {
  ...
  return (
    <div className="flex flex-col w-full px-4 sm:px-6 lg:px-8 space-y-6 max-w-[1400px] mx-auto pb-6 pt-1 lg:pt-2">
      ...
      {/* Main Responsive Grid Layout on Desktop */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* Right Column: Verified Spots Grid */}
          ...
          {filteredSpots.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {filteredSpots.map((spot) => (
                <div
                  key={spot.id}
                  className="p-4 rounded-2xl bg-white shadow-xs flex flex-col justify-between space-y-3 border border-gray-200 hover:border-gray-300 transition-all"
                >
                  <div className="flex items-start gap-3">
                    <div className="w-18 h-18 rounded-xl overflow-hidden bg-gray-100 shrink-0 shadow-xs relative border border-gray-100">
                      <img
